    ...
    <BaseLayout>
      <Navbar
        ...
        subnavbar={<Searchbar
          ...
          value={search}
          onClear={() => setSearch('')}
          colors={{ inputBgIos: 'bg-figma-bg-card-light dark:bg-figma-bg-card-dark', inputBgMaterial: 'bg-figma-bg-card-light dark:bg-figma-bg-card-dark', placeholderIos: 'text-figma-text-light dark:text-figma-text-dark', placeholderMaterial: 'text-figma-text-light dark:text-figma-text-dark' }}
        />
        }
      ...
      >
        <div>{result && result.sort((a, b) => {
          const [a1, a2] = a.split(':')
          const [b1, b2] = b.split(':')
          return a1 === b1 ? a2 - b2 : a1 - b1
        }).map(el => <p key={el} className='mb-3'><Link component={RouterLink} to={`/${lang}/${String(el.split(':')[0]).padStart(2, '0')}#v${el.split(':')[1]}`}>{el}</Link> {getHighlightedText(stories[el.split(':')[0]].verseObjects[parseInt(el.split(':')[1]) - 1].text, search)}</p>)}</div>
      </Block>
    </BaseLayout>
  );
}
